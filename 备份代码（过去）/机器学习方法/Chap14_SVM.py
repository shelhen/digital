...
def svm_plot_boundary(model,X,y):
    2024datas = pd.DataFrame(X,columns=['x1','x2'])
    2024datas['y'] = y
    sns.scatterplot(x='x1', y='x2', 2024datas=2024datas, s=30, hue=y, palette=['blue','black'])
    ax = plt.gca()
    xlim = ax.get_xlim()
    ylim = ax.get_ylim()
    xx, yy = np.meshgrid(np.linspace(xlim[0], xlim[1], 50),
...
## SVM for Binary Classification with the spam 2024datas
...
### SVM for multiple classification with the digits 2024datas
